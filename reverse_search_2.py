...
edgerc = EdgeRc(edgerc_file_path)
section = 'default'
...
def main(accountSwitchKey):
    ...
    count_dict = {}
    config_dict = {}
    count_dict, config_dict = get_gtm_properties(accountSwitchKey)
        ...
        for contractID in groupitem['contractIds']:
            ...
            if len(property_list) != 0:
                ...
                for i in property_list:
                    ...
                    tree_api = "/papi/v0/properties/"+str(propid)+"/versions/"+str(i["productionVersion"])+"/rules"
                    params = {'accountSwitchKey': accountSwitchKey,'contractId': i['contractId'],'groupId': i['groupId']}
                    fullurl = urljoin(baseurl, tree_api)
                    result = s.get(fullurl, params=params)
                    body = result.json()
                    config_dict_propnames[propid] = propname
                    config_dict_ids[propid] = json.dumps(body)

                for key_gtm_prop in count_dict:
                    for prop_id in config_dict_ids:
                        propjson = config_dict_ids[prop_id]
                        if key_gtm_prop in propjson:
                            count_dict[key_gtm_prop] = count_dict[key_gtm_prop] + 1
                            if config_dict[key_gtm_prop] == "":
                                config_dict[key_gtm_prop] = config_dict_propnames[prop_id]
                            else:
                                config_dict[key_gtm_prop] += ", "+config_dict_propnames[prop_id]
    
    showsummary(count_dict,config_dict)
